...
export default function Request({ data }: { data: any }) {
  ...
  function btnHelpClick() {
    const donationInBnbString = prompt("O quanto deseja doar (em BNB)?", "0"); // Corrigido para string
    const donationInBnb = donationInBnbString ? parseFloat(donationInBnbString) : 0; // Converte para número

    // Verifica se a conversão foi bem-sucedida e o valor é maior que 0
    if (isNaN(donationInBnb) || donationInBnb <= 0) {
